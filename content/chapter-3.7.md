# Chapter 3.7 - Documentation Consolidation Plan
...
### 3.7.2 Source Material to Consolidate
...
| Chapter 3.3 | Object, Subject, Vector, Boundary, Seed ontology | Chapter 3 ontology |
| Chapter 3.4 | A.A.S. system architecture and Field Runtime | Chapter 3 architecture |
| Chapter 3.5 | Frontend workspace architecture | Chapter 3 frontend |
| Chapter 3.6 | Backend truth, commands, events, APIs, storage, recovery | Chapter 3 backend |
...
### 3.7.3 Source Preservation Register
...
| Four workspaces: Chat, Draw, Model, Architect | Chapter 1 capabilities, Chapter 3.2 workspace model, Chapter 3.5 frontend |
| Five-node ontology: Object, Subject, Vector, Boundary, Seed | Chapter 1 design model and Chapter 3.3 canonical ontology |
| Design pipeline `Object + Subject -> Vector + Boundary -> Seed` | Chapter 1 introduction and Chapter 3.3 ontology |
| Agent roles: Planner, Research, Concept, Design Development, Model, Representation, Critic/Evaluator, Supervisor, Curator | Chapter 3.4 agent and service roles |
| Runtime services: AffordanceCompiler, ContextDistiller, MoveCompiler, FeatureRegistry, EvaluatorRegistry, TensionEngine, BranchEcology, CommitmentLedger | Chapter 3.4 Field Runtime and Chapter 3.6 backend services |
| Core runtime objects: WorldState, DirectionNode, DirectionLink, DirectionSnapshot, ConceptState, ValueVector, Affordance, Move, Command, AgentPatch, MovePrimitive, MovePattern, IntentGradient, FeatureRegistry, Tension, Branch, Commit, AgentBrief, Trajectory | Chapter 3.4 Core Runtime Object Catalog |
...
| Process grammar, trajectory memory, design debt, unlock value, landmarks | Chapter 3.4 object catalog and Field Runtime architecture; Chapter 4 where work is phased |
| Branch lifecycle: spawn, develop, critique, compare, kill, merge, commit | Chapter 3.4 and Chapter 4 |
| Tension detection, tension energy, blocking finalization, artifact links | Chapter 3.4 and Chapter 3.6 |
| Commitment contents, revert rule, downstream enforcement | Chapter 3.4 and Chapter 3.6 |
| Event names, replay, reconnect, undo, audit | Chapter 3.6 backend event system |
| API surfaces for world, commands, direction graph, affordances, moves, move patterns, features, tensions, branches, commits, patches, briefs, bridge | Chapter 3.6 API surface |
| External tools: Rhino Compute, GPT Image V2, renderers, validators, exporters | Chapter 3.4 integration rule and Chapter 4 |
| Anti-patterns: graph-first product, raw tool exposure, memory as truth, uncommitted truth, hidden mutation, unregistered score variables, unvalidated learned moves, ontology drift | Distributed to Chapter 1 hardlines, Chapter 2 anti-patterns, Chapter 3.3 ontology rules, Chapter 3.4 ownership, and Chapter 4 |

### 3.7.4 Consolidation Rules
...
### 3.7.5 Conflicts to Resolve Into Final Decisions
...
3. Keep Chapter 3.3 as the canonical ontology chapter.
4. Keep Chapter 3.4 as the final A.A.S. system architecture and Field Runtime overview.
5. Keep Chapter 3.5 as the final frontend/workspace architecture.
6. Keep Chapter 3.6 as the final backend control-plane architecture.
7. Remove implementation-plan material from the final Chapter 3 reader path.
8. Keep Chapter 3.7 as a temporary consolidation guide until the final chapter set is stable.
9. Keep Chapter 4 as the only reader-facing implementation-plan chapter.

### 3.7.7 Chapter 4 Role
